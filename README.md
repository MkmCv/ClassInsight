...
> 视觉算法（YOLO-vHeat，基于 ultralytics 二次开发并集成 vHeat 模块）已拆分为独立仓库 [YOLO12-vHeat](https://github.com/MkmCv/YOLO12-vHeat)，在本项目中以 **git 子模块** 形式引入（`model/yolo-vheat`）。
...
├── model/                    # 视觉算法
│   └── yolo-vheat/           # git 子模块 → YOLO12-vHeat（供后端 ultralytics 可编辑安装）
...
```bash
# 克隆时一并拉取 YOLO-vHeat 子模块
git clone --recurse-submodules https://github.com/MkmCv/ClassInsight.git
...
1. 确认子模块已拉取：`git submodule update --init --recursive`（`model/yolo-vheat` 不应为空）。
2. 在 `model/yolo-vheat` 执行 `pip install -e .`。若 `model/` 目录位置变化，可编辑安装的旧路径会失效，需重新安装。
</details>
...
- 视觉算法 [YOLO12-vHeat](https://github.com/MkmCv/YOLO12-vHeat) 作为独立仓库维护，并以 git 子模块引入；本仓库聚焦系统侧（前后端）。
