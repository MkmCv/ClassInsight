...
答辩可简述：单机部署、免独立数据库服务、适合毕设演示；生产若需可更换为 PostgreSQL 等，主要改连接串与驱动。
...
3. **建表**：`init_db()` 中 **先 `import ..models` 注册所有 ORM**，再对 `Base.metadata` 执行 **`create_all`**。  
   ...
   - **结构演进**：仓库中的 `migrate_*.py` 等脚本用于手工补列/补表（按需执行）。
...
*可与 `TIME_AGGREGATION.md`、`教学建议与优化-答辩说明.md` 一并作为答辩材料。*
