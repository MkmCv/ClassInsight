...
def main(config):
    # 主流程：构建数据 → 模型（含 EMA）→ 优化器/调度器/损失 → 训练/验证 → 保存最优权重
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler, loss_scaler, model_ema=None):
    """
    单轮训练：
      - AMP autocast + 梯度缩放（loss_scaler）
      - 支持梯度累积（ACCUMULATION_STEPS）
      - 可选 Mixup/Cutmix（通过外部 mixup_fn 已在 dataloader 阶段设置）
      - EMA（若开启）在每步更新
    """
    model.train()
    optimizer.zero_grad()
...
def validate(config, data_loader, model):
    """
    验证：不启用梯度，统计时间、Loss、Top-1/Top-5 准确率，并在多卡上做 reduce。
    """
    criterion = torch.nn.CrossEntropyLoss()
    model.eval()
...
if __name__ == '__main__':
    ...
    # DDP 初始化：从环境变量获取 rank/world_size，设置本地设备与进程组
